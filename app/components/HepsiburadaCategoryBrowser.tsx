...
export default function HepsiburadaCategoryBrowser({
  categoryTree,
  onSelect,
  onSearch,
  selectedCategoryId,
  ...
  // Render a tree node
  const renderTreeNode = useCallback((node: CategoryTreeNode, depth: number = 0): React.ReactNode => {
    const nodeKey = node.pathString;
    const isExpanded = expandedNodes.has(nodeKey);
    const hasChildren = node.children && node.children.length > 0;
    const isSelected = node.leaf && node.categoryId === selectedCategoryId;
    const isVirtual = node.categoryId === 0; // Virtual parent nodes
      ...
      <div key={nodeKey} className="select-none">
        <div
          className={`
            ...
            ${isSelected ? 'bg-amber-100 border border-amber-300' : 'hover:bg-gray-100'}
            ${depth > 0 ? 'ml-' + Math.min(depth * 4, 12) : ''}
          `}
          style={{ marginLeft: depth * 16 }}
          onClick={() => {
            if (node.leaf && node.categoryId !== 0) {
              handleSelectCategory(node.categoryId, node.displayName || node.name, node.pathString);
            } else if (hasChildren) {
              toggleNode(nodeKey);
            }
          }}
    ...
    return (
      <div className="space-y-1">
        {searchResults.map(result => {
          const isSelected = result.categoryId === selectedCategoryId;
          return (
            <div
              key={result.categoryId}
              className={`
                px-3 py-3 rounded-lg cursor-pointer transition-colors
                ${isSelected ? 'bg-amber-100 border border-amber-300' : 'hover:bg-gray-100 border border-transparent'}
              `}
          ...
          {mode === 'search' && (
            <div className="flex-1 relative">
              <input
                ...
                className="w-full pl-3 pr-10 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500"
                autoFocus
              />
              {searchQuery && (
                <button
